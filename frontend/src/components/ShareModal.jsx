...
const ShareModal = ({ project, onClose }) => {
  ...
  const copyInviteLink = () => {
    if (!inviteDetails?.accept_url) return;
    navigator.clipboard.writeText(inviteDetails.accept_url);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };
            ...
            <form onSubmit={handleInvite} className="space-y-4">
              ...
              {inviteDetails?.accept_url && (
                <div className="space-y-3 rounded-lg border border-slate-700 bg-slate-800/60 p-3">
                  ...
                  <input
                    readOnly
                    value={inviteDetails.accept_url}
                    className="w-full px-3 py-2 bg-slate-950 border border-slate-700 rounded-lg text-white font-mono text-xs focus:outline-none"
                  />
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      type="button"
                      onClick={copyInviteLink}
                      className="flex items-center justify-center gap-2 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm transition-colors"
                    >
                      {copied ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                      Copy link
                    </button>
                    <a
                      href={inviteDetails.mailto_url}
                      className="flex items-center justify-center gap-2 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg text-sm transition-colors"
                    >
